...
class ContratSimpleMigrationGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Migration Contrat (avec Ecole)")
    ...
    def setup_ui(self):
        ...
        title_label = ctk.CTkLabel(
            self.root,
            text="🔄 Migration Contrat (avec Ecole)",
            font=("Arial", 24, "bold")
        )
    ...
    def start_migration(self):
        ...
        if not self.contrat_file:
            messagebox.showerror("Erreur", "Veuillez sélectionner le fichier contrat.csv")
            return
            ...
            with open(self.contrat_file, 'r', encoding='latin-1') as csvfile:
                ...
                for row_num, row in enumerate(reader, 1):
                    try:
                        ...
                        type_personne = row[4].strip() if len(row) > 4 else ''
                        ...
                        diplome = row[8].strip() if len(row) > 8 else ''
                        ecole = row[9].strip() if len(row) > 9 else ''
                        
                        # Remplacer les valeurs par défaut/vides par "Non Renseigné"
                        ecole_lower = ecole.lower() if ecole else ""
                        if not ecole or ecole_lower == "sélectionnez une école" or "sélectionnez" in ecole_lower or "selectionnez" in ecole_lower:
                            ecole = "Non Renseigné"
...
if __name__ == "__main__":
    ...
    root = ctk.CTk()
    app = ContratSimpleMigrationGUI(root)
    root.mainloop()
