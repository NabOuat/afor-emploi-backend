...
from app.models import TRegion, TDepartement, TSousPrefecture, Users
...
async def get_regions(db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
    return db.query(TRegion).all()

@router.get("/regions/{region_id}", response_model=TRegionSchema)
async def get_region(region_id: str, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def create_region(region: TRegionCreate, db: Session = Depends(get_db), _: Users = Depends(require_admin)):
...
async def update_region(region_id: str, region: TRegionCreate, db: Session = Depends(get_db), _: Users = Depends(require_admin)):
...
async def delete_region(region_id: str, db: Session = Depends(get_db), _: Users = Depends(require_admin)):
    db_region = db.query(TRegion).filter(TRegion.id == region_id).first()
    if not db_region:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Region not found")
    db.delete(db_region)
...
async def get_departements(region_id: str = None, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def get_departement(departement_id: str, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def create_departement(departement: TDepartementCreate, db: Session = Depends(get_db), _: Users = Depends(require_admin)):
...
async def update_departement(departement_id: str, departement: TDepartementCreate, db: Session = Depends(get_db), _: Users = Depends(require_admin)):
...
async def delete_departement(departement_id: str, db: Session = Depends(get_db), _: Users = Depends(require_admin)):
    db_departement = db.query(TDepartement).filter(TDepartement.id == departement_id).first()
    if not db_departement:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Departement not found")
...
async def get_sousprefectures(departement_id: str = None, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def get_sousprefecture(sousprefecture_id: str, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def create_sousprefecture(sousprefecture: TSousPrefectureCreate, db: Session = Depends(get_db), _: Users = Depends(require_admin)):
...
async def update_sousprefecture(sousprefecture_id: str, sousprefecture: TSousPrefectureCreate, db: Session = Depends(get_db), _: Users = Depends(require_admin)):
...
async def delete_sousprefecture(sousprefecture_id: str, db: Session = Depends(get_db), _: Users = Depends(require_admin)):
    db_sousprefecture = db.query(TSousPrefecture).filter(TSousPrefecture.id == sousprefecture_id).first()
    if not db_sousprefecture:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Sous-prefecture not found")
...
async def download_template(type: str, _: Users = Depends(get_current_user)):
...
async def import_geo_csv(type: str, file: UploadFile = File(...), db: Session = Depends(get_db), _: Users = Depends(require_admin)):
