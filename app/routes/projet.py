...
from app.models import Projet, Users
...
async def get_projets(db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
    return db.query(Projet).all()

@router.get("/{projet_id}", response_model=ProjetSchema)
async def get_projet(projet_id: str, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def create_projet(projet: ProjetCreate, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
    db_projet = Projet(
        id=str(uuid.uuid4()),
        nom=projet.nom,
...
async def update_projet(projet_id: str, projet: ProjetCreate, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def delete_projet(projet_id: str, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
    db_projet = db.query(Projet).filter(Projet.id == projet_id).first()
    if not db_projet:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Projet not found")
    db.delete(db_projet)
