...
from app.models import FicPersonneLocalisation, Contrat, Users
...
async def get_localisations(contrat_id: str = None, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def get_localisation(localisation_id: str, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def create_localisation(localisation: FicPersonneLocalisationCreate, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def update_localisation(localisation_id: str, localisation: FicPersonneLocalisationCreate, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def delete_localisation(localisation_id: str, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
    db_localisation = db.query(FicPersonneLocalisation).filter(FicPersonneLocalisation.id == localisation_id).first()
    if not db_localisation:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Localisation not found")
...
async def change_employee_location(employee_id: str, localisation: FicPersonneLocalisationCreate, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
