...
from app.models import FicPersonne, Acteur, Projet, Users
...
async def get_personnes(db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
    return db.query(FicPersonne).all()

@router.get("/{personne_id}", response_model=FicPersonneSchema)
async def get_personne(personne_id: str, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def create_personne(personne: FicPersonneCreate, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def update_personne(personne_id: str, personne: FicPersonneCreate, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
...
async def delete_personne(personne_id: str, db: Session = Depends(get_db), _: Users = Depends(get_current_user)):
    db_personne = db.query(FicPersonne).filter(FicPersonne.id == personne_id).first()
    if not db_personne:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Personne not found")
